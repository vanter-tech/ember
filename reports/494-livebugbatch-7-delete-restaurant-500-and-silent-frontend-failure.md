...
- **Predecessor task:** report 493 (bug 6 — ADMIN sees "no open shift" in `/waiter/tables`)
...
## 3. Modified Files
- `backend/src/main/resources/db/migration/V12__widen_restaurants_status_check.sql` (new)
...
New `V12__widen_restaurants_status_check.sql`, identical idempotent `DROP CONSTRAINT IF EXISTS` +
...
All 7 live bug reports from this session are fixed (reports 488-494, commits on
...
7. Restaurant deletion actually works (`V12` migration) and failures are no longer silent.
