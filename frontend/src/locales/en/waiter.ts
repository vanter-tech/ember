import type { waiter as esWaiter } from '../es/waiter'

export const waiter = {
  loadingDashboard: 'Loading dashboard data...',
  mainRoomTitle: 'Main Room',
  statusOccupied: 'Occupied',
  statusFree: 'Free',
  needOpenCajaOverlay: 'You need to open the register to assign a table.',
  tablesEmptyTitle: 'No tables set up yet',
  tablesEmptyDescription: 'Ask an admin to add tables from the restaurant settings.',
  tableDetailsTitle: 'Table details',
  waiterLabel: 'Waiter',
  unassignedLabel: 'Unassigned',
  noOrderCurrently: 'No order currently',
  openTableButton: 'Open table',
  viewInfoButton: 'View Information',
  printBillLabel: 'Print Bill',
  transferLabel: 'Transfer',
  addItemLabel: 'Add Item',
  assignTableLabel: 'Assign Table',
  selectTablePrompt: 'Select a table to see the details',
  tableHeading: 'Table M{{number}}',
  waiterRoleLabel: '(WAITER)',
  orderDetailsTitle: 'Order details',
  noOrdersRegistered: 'No orders registered for this table.',
  participantsTitle: 'Participants',
  noUsersInTable: 'No users at this table.',
  activityTitle: 'Activity',
  itemDeletedLabel: '{{itemName}} was deleted',
  deletedLabel: 'Deleted',
  orderPlacedLabel: 'Order placed',
  participantLeftLabel: '{{name}} left the table',
  tableOpenedAtLabel: 'Table opened: {{timestamp}}',
  waiterIdLabel: 'Waiter: {{waiterId}}',
  billTitle: 'Bill',
  summaryTitle: 'Summary',
  voidBillLabel: 'Void Bill',
  paidLabel: 'Paid',
  partiallyPaidLabel: 'Partial payment',
  confirmDigitalButton: 'Confirm digital',
  markPaidButton: 'Mark as paid',
  totalLabel: 'Total',
  subtotalLabel: 'Subtotal',
  taxesLabel: 'Taxes ({{rate}}%)',
  chargeMesaLabel: 'Charge Table',
  waitingForDeliveryLabel: 'Waiting for order delivery',
  closingTableLabel: 'Closing',
  closeTableButton: 'Close table',
  loadingCashRegister: 'Loading register...',
  cashRegisterTitle: 'Cash register',
  cashRegisterSubtitle: 'Opening, movements, and shift reconciliation.',
  tourCashRegisterTitle: 'Your shift',
  tourCashRegisterContent: 'Open your shift, record cash movements, and close it out at the end of the day from here.',
  noOpenShift: 'There is no open cash shift.',
  openCajaButton: 'Open register',
  adminCashRegisterReadOnly: 'Only a waiter can open, close, or modify the register. Your admin session can only view it.',
  shiftNumberLabel: 'Shift #{{number}}',
  shiftStatusOpen: 'Open',
  shiftStatusClosed: 'Closed',
  openingFloatLabel: 'Opening float',
  openedByLabel: 'Opened by',
  recordMovementButton: 'Record movement',
  closeCajaButton: 'Close register (Reconciliation)',
  cashSalesLabel: 'Cash sales',
  cashInTotalLabel: 'Cash in',
  cashOutTotalLabel: 'Cash out',
  expectedCashLabel: 'Expected cash in register',
  expectedCashHint: 'Float + cash sales + cash in − cash out',
  openedAtLabel: 'Opened',
  deadlineLabel: 'Deadline',
  timeLeftLabel: '{{time}} left',
  shiftOverdueBadge: 'Overdue',
  timeColumnLabel: 'Time',
  tableColumnLabel: 'Table',
  methodLabel: 'Method',
  methodCash: 'Cash',
  methodDigital: 'Digital',
  statusLabel: 'Status',
  paymentPending: 'Pending',
  paymentConfirmed: 'Confirmed',
  refundedTotalLabel: 'Refunded this shift',
  movementsTitle: 'Movements',
  typeLabel: 'Type',
  amountLabel: 'Amount',
  reasonLabel: 'Reason',
  registeredByLabel: 'Registered by',
  noMovementsRegistered: 'No movements registered.',
  cashInLabel: 'Cash in',
  cashOutLabel: 'Cash out',
  paymentsTitle: 'Payments',
  participantLabel: 'Participant',
  refundedLabel: 'Refunded',
  actionLabel: 'Action',
  noPaymentsRegistered: 'No payments registered this shift.',
  refundButton: 'Refund',
  closeShiftTitle: 'Shift reconciliation',
  closeShiftDescription:
    'Count the cash in the register and enter the total. The system will show the difference after recording the count.',
  closeNotesLabel: 'Notes (optional)',
  closeNotesPlaceholder: 'Explain the difference, if any',
  cancelButton: 'Cancel',
  closingShiftLabel: 'Closing...',
  confirmCountButton: 'Confirm count',
  expectedLabel: 'Expected',
  countedLabel: 'Counted',
  differenceLabel: 'Difference',
  closeButton: 'Close',
  movementDialogTitle: 'Manual movement',
  movementSavingLabel: 'Saving...',
  registerButton: 'Register',
  openShiftTitle: 'Register opening',
  openingLabel: 'Opening...',
  chargeMesaDescription: 'Choose how the bill will be split among participants.',
  byConsumptionLabel: 'By consumption',
  byConsumptionDescription: 'Each participant pays for what they ordered.',
  equalPartsLabel: 'Equal parts',
  equalPartsDescription: 'The total is split among {{count}} participant(s).',
  calculatingLabel: 'Calculating...',
  confirmChargeButton: 'Confirm and calculate bill',
  assignTableDescription: 'Configure the table details before opening it',
  selectParticipantCountLabel: 'Select the number of guests for the table.',
  qrPlaceholderLabel: 'The QR code will appear here.',
  joinCodeLabel: 'Code to join the table:',
  tableQrInfoLabel: 'Table code & QR',
  qrLoadingLabel: 'Generating code...',
  qrSavingLabel: 'Saving',
  openTableGenerateQrButton: 'Open Table and Generate QR',
  seatNamesLabel: 'Seat names (optional)',
  seatNamePlaceholder: 'Seat {{n}}',
  addSeatLabel: 'Add seat',
  renameSeatTitle: 'Rename seat',
  removeSeatTitle: 'Remove seat',
  seatNameInputLabel: 'Seat name',
  seatSaveButton: 'Save',
  seatRemoveConfirmButton: 'Remove',
  removeSeatConfirm: 'Remove seat "{{name}}"? Its draft items are discarded.',
  seatAddedToast: 'Seat added',
  seatRenamedToast: 'Seat renamed',
  seatRemovedToast: 'Seat removed',
  seatErrorToast: "Couldn't update the seat",
  renameBlockedBillExistsToast: 'Recalculate the bill before renaming seats',
  refundPaymentTitle: 'Refund Payment',
  availableBalanceLabel: '· available balance: {{amount}}',
  refundAmountPlaceholder: 'Amount to refund',
  refundReasonPlaceholder: 'Refund reason',
  voidBillDescription: 'The calculated bill is voided and the table becomes free to recalculate.',
  voidReasonPlaceholder: 'Void reason',
  voidButton: 'Void',
  shiftOpenedToast: 'Shift opened successfully.',
  shiftOpenErrorToast: 'Could not open the shift.',
  movementAmountPositiveError: 'Amount must be greater than zero',
  movementReasonRequiredError: 'Enter a reason',
  movementRegisteredToast: 'Movement recorded.',
  movementErrorToast: 'Could not record the movement.',
  shiftCloseErrorToast: 'Could not close the shift.',
  shiftCloseTablesOpenToast: 'Cannot close the shift: {{count}} table(s) still have an open session.',
  cannotRemoveSentItem: 'Cannot remove: this item has already been sent to the kitchen.',
  tableClosedPaidToast: 'Table paid and closed.',
  tablePaidClosedBanner: 'Table paid and closed. You can print the bill before leaving.',
  tableClosedNoOrderBanner: 'Table freed up: no order was placed.',
  printSentToast: 'Bill sent to the printer',
  printQueuedNoAgentToast: 'Bill queued (no printer connected)',
  printFailedToast: 'Could not print the bill',
  tableClosedToast: 'Table closed.',
  cashPaymentRegisteredToast: 'Cash payment recorded.',
  cashPaymentErrorToast: 'Could not record the payment. Is a shift open?',
  digitalPaymentConfirmedToast: 'Digital payment confirmed.',
  digitalPaymentErrorToast: 'Could not confirm the digital payment.',
  billVoidedToast: 'Bill voided.',
  billVoidErrorToast: 'Could not void the bill.',
  removeDinerLabel: 'Remove diner and split their share among those present',
  splitRedistributedToast: '{{name}} left the table. Their share was split among those present.',
  splitRedistributedDoneToast: 'Share redistributed among the diners present.',
  splitRedistributeErrorToast: "Could not redistribute that diner's share.",
  settleAndCloseButton: 'Settle and close table',
  tableSettledToast: 'Table settled and closed.',
  tableSettleErrorToast: 'Cannot close: some shares are still unpaid. Redistribute or collect them first.',
  refundRegisteredToast: 'Refund recorded.',
  refundErrorToast: 'Could not record the refund. Is a shift open?',
  tableOpenedToast: 'Table opened successfully',
  billRequestedToast: 'Bill requested.',
  billCalculateErrorToast: 'Could not calculate the bill.',
  tourGridTitle: 'Your tables',
  tourGridContent: 'Here you see every table in the restaurant. Red means occupied, gray means free.',
  tourPanelTitle: 'Table details',
  tourPanelContent: "Click a table and you'll see who's seated there and what they've ordered.",
  tourAssignTitle: 'Invite guests',
  tourAssignContent: "Generates a QR code so this table's guests can join from their phone.",
  tourTableActionsTitle: 'Table actions',
  tourTableActionsContent:
    'Print the bill, transfer the table to another waiter, or add an item to the order manually.',
  tourTableOrdersTitle: 'Order details',
  tourTableOrdersContent:
    "Everything the guests have ordered. You can remove an item as long as it hasn't been sent to the kitchen.",
  tourTableParticipantsTitle: 'Participants',
  tourTableParticipantsContent: 'The guests who joined this table from their phones.',
  tourTableActivityTitle: 'Activity',
  tourTableActivityContent:
    'Table history: when it opened, what was ordered, and what was removed.',
  tourTableBillTitle: 'Bill and close-out',
  tourTableBillContent:
    'The summary or the bill with its splits. From here you charge the table and close it.',
  cashShiftPreWarningTitle: 'The cash register is about to close',
  cashShiftPreWarningBody: 'The register closes at {{time}}. Close it or extend it.',
  cashShiftOverdueTitle: 'The cash register has expired',
  cashShiftOverdueBody:
    'The register expired at {{time}}. Cash payments and movements are blocked until you close it. Table service still works.',
  cashShiftStaleTitle: 'The {{date}} register was never closed',
  cashShiftStaleBody:
    'Everything recorded since then is being added to the {{date}} shift. Close it before starting the day.',
  cashShiftStaleMovementsCount: '{{count}} movement(s) accumulated',
  cashShiftStalePaymentsCount: '{{count}} payment(s) accumulated',
  cashShiftStaleCloseButton: 'Close the {{date}} register',
  cashShiftProlongButton: 'Extend 1 h',
  cashShiftCloseButton: 'Close register',
  cashShiftLaterButton: 'Not now',
  cashShiftProlongedToast: 'Register extended by one hour.',
  cashShiftProlongErrorToast: 'Could not extend the register.',
  cashShiftOpenTodayButton: "Open today's shift",
  cashShiftOverduePaymentToast: 'The register expired. Extend or close it to charge.',
  cashShiftOverdueMovementBlocked: 'The register expired. Extend or close it to record movements.',
  addItemModalTitle: 'Add item',
  addItemSearchPlaceholder: 'Search item...',
  addItemParticipantLabel: 'Assign to',
  addItemParticipantMesa: 'Table (shared)',
  addItemUncategorized: 'Uncategorized',
  addItemCartHeading: 'Order for {{name}}',
  addItemEmptyCart: 'No items yet for this customer.',
  addItemViewCartButton: 'View order',
  addItemRemoveLineAria: 'Remove {{name}}',
  addItemCartPanelCloseAria: 'Close cart',
  addItemAddAria: 'Add {{name}}',
  addItemModifierCancel: 'Cancel',
  addItemModifierConfirm: 'Accept',
  addItemSubmit: 'Confirm order',
  addItemSuccessToast: 'Item added and sent to the kitchen',
  addItemErrorToast: 'Could not add the item',
  addItemBillExistsToast: 'The bill was already requested. Void it to add items.',
  transferModalTitle: 'Transfer table',
  transferModalDescription: 'Pick the waiter who will take over this table.',
  transferNoWaiters: 'No other active waiters',
  transferSubmit: 'Transfer',
  transferSuccessToast: 'Table transferred to {{name}}',
  transferErrorToast: 'Could not transfer the table',
  billsLabel: 'Bills',
  coinsLabel: 'Coins',
  totalCountedLabel: 'Total counted',
  billDenominationLabel: 'Bill {{amount}}',
  coinDenominationLabel: 'Coin {{amount}}',
} satisfies typeof esWaiter
