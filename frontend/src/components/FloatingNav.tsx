...
export const FloatingNav = () => {
    ...
    return(
        ...
        px-4 py-2 flex items-center gap-2 border border-zinc-200 dark:border-zinc-800 z-50">
            ...
            {role === 'ADMIN' && (
                <>
                <Link to="/admin/categories" className={navItemClass('/admin/categories')} title="Categorias">
                    <Package strokeWidth={1.5} size={24} />
                    
                </Link>
                <Link to="/admin/reports" className={navItemClass('/admin/reports')} title="Reportes">
                    <BarChart3 strokeWidth={1.5} size={24} />
                </Link>
                <div className="w-px h-8 bg-zinc-200 dark:bg-zinc-700 mx-2"></div>
                <Link to="/admin/settings" className={navItemClass('/admin/settings')} title="Configuración">
            ...
            border-zinc-200 dark:border-zinc-700">
                ...
                <button onClick={handleLogout} className="text-[#920703]
                hover:text-red-600 transition-colors cursor-pointer" title="Cerrar sesión">
