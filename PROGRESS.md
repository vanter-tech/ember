...
- **Last Completed Task:** report 544 — **WAITER-ADD-ITEM-REVIEW-FEEDBACK** (branch `fix/waiter-add-item-modal-review-feedback` off `main`, not pushed) — user review of r543's split view asked for 4 things: bigger modal (`sm:max-w-3xl`→`6xl`, now the largest in the codebase), more gap + a border between the client/product columns, dish photos (`MenuItemResponse.imageUrl`, placeholder icon when absent), and a dedicated 44px "+" per card instead of the whole card being clickable (misclick guard on a touch screen). TDD: rewrote the interaction tests to target the "+" button, added a misclick-guard test + a photo/placeholder test, RED confirmed (8 failures) against r543's component first. One real type error caught by `tsc -b` (`item.name` possibly undefined) and fixed. `pnpm run test:run` **190/190**, `build`/`lint` clean. **Not done:** a live browser check (needs an authenticated waiter session with an open table — judged disproportionate for a styling refinement already covered by 13 behavioral tests). **Pending (user-gated):** PR/merge, then rebuild `EmberHubSetup-0.3.0.exe` again (first rebuild after r543 succeeded — `D:\Vanter\ember\ember-hub\dist\EmberHubSetup-0.3.0.exe`, 191MB, local only, not yet published — hit and fixed a stale-path Rust build-cache issue unrelated to the app code, `cargo clean` in `ember-hub/src-tauri` before rebuilding solved it).
- **Prev (compressed, reports 540-542 — three independent `landing/`-only tasks, merged to `main` locally 2026-09-22):** PRICING-TABLE-HONESTY (r540) — `/planes` audited against `PlanGateService`'s 6 real gates, 3 fabricated rows removed, 5 falsely-gated rows fixed. DOWNLOADS-BUTTON-ALIGNMENT (r541) — `mt-auto` fix for the Hub/agent download CTAs. HUB-PRICE-MODAL (r542) — "Ver precio" button + `<dialog>` on `LocalPlan.astro` showing Hub's list price ($490/año, $294/semestre). All `pnpm run build` clean. **Pending (user-gated):** push to `origin`. Also this session: defined Ember Hub's pricing + sales cadence (semestral first for client #1, to maximize close probability; 30% founder discount first 12 months only, never published) — see memory `ember-hub-pricing`; paused a prod-infra hardening brainstorm before writing its spec — see memory `ember-prod-infra-hardening`.
