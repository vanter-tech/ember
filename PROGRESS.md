...
- **Last Completed Task:** report 548 — **ADMIN-PASSWORD-RESET (F-25)** — operator-assisted password recovery for a locked-out restaurant ADMIN (Console "Resetear contraseña" → temp password + forced change-on-next-login modal). No mail infra exists in the backend, so self-service email reset was evaluated and deferred in favor of this smaller, already-fitting pattern. `./mvnw test` **1540/1540**, `pnpm run test:run` **198/198** (54 files), `build`/`lint` clean. Commit/push pending (user-gated, next tool call).
- **Prev (compressed, r540-547, 2026-09-23 push/tag/deploy round):** WAITER-ADD-ITEM review passes 1-4 (r544-547, cart panel became a real `DialogContent` child, fixed the close-everything bug). Pushed `main` to `origin` (r540-547). Discovered `v0.3.0` was tagged but never rolled out via `deploy.sh`; bumped `backend/pom.xml` → tagged/pushed **`v0.3.1`**. **Pending (user-gated, Cloud Shell):** `./deploy/deploy.sh 0.3.1` to actually roll out to the VM; `EmberHubSetup-0.3.0.exe` built locally, not published.
- **Prev (compressed, reports 534-542 — security-debt batch + RESTAURANT-DEPLOYMENT-MODE + landing polish):** F-21/F-14/F-15/F-17/F-10 security fixes + RESTAURANT-DEPLOYMENT-MODE `V15` (r534-539); PRICING-TABLE-HONESTY, DOWNLOADS-BUTTON-ALIGNMENT, HUB-PRICE-MODAL (r540-542). Also this session: defined Ember Hub's pricing — see memory `ember-hub-pricing`; corrected the legal identity text on `privacy`/`terms` (Vanter = trade name of Fernando Obando, persona natural, no persona jurídica yet) + drafted `docs/legal/2026-09-23-pilot-service-agreement.md`; paused a prod-infra hardening brainstorm — see memory `ember-prod-infra-hardening`.
- **Current Active Task:** none in code. Next up (same workflow-review thread): **HUB-LOCAL-PRINTING** — Hub detects/prints to local printers in-process instead of requiring the separate print-agent (see its section below). One task per context — `/clear` before starting it. Prod-infra hardening (rollback rehearsal + restore test, see memory `ember-prod-infra-hardening`) and `deploy.sh 0.3.1` remain open, user-directed.
...
- **System Health:** backend `./mvnw test` **1540/1540** (this report). `frontend`: `pnpm run test:run` **198/198** (54 files), `build`/`lint` clean (this report). `landing`: `pnpm run build` clean, 28 pages (this session's legal-text edit, unrelated to this task). `ember-hub/ui` last verified in the r534-539 batch.
...
- Next: point 2 from the same workflow review — Hub local printing without the separate print-agent (see `HUB-LOCAL-PRINTING` below), separate context per `CLAUDE.md` §7
