...
- **Current Active Task:** **EMB-PRINT-AGENT** — plan `docs/superpowers/plans/2026-09-08-print-agent-installer.md` (7 tasks T1–T7). **T1 done (report 417).** Next: **T2 (agent — observable `StatusHub` + DPAPI credential store + config precedence; adds `jna-platform` dep; verify cmd is `mvn -f printing-agent/pom.xml test`)**. Do one task per context, `/clear` between (CLAUDE.md §7). Backend **`v0.2.2` deployed to prod** (2026-09-08): reports 409–410 + guest-join + Hub seats + optimistic-lock 409; V9 applied clean, app healthy. Reports 411–414 merged to `main` (frontend auto-deploys). Report 415 (SaaS favicon distinct) on branch `fix/saas-favicon-distinct` — PR pendiente. **Landing contact form is LIVE and verified** — real submission on `ember.vanter.net/contacto` → `/gracias` → email received at `tofernandoband01@outlook.com` (2026-09-08). PRs #100–#102 all merged (`f37eb8d1`). Owner has the 2 Worker Secrets set. Optional follow-ups: verify `vanter.net` in Resend for a `contacto@vanter.net` sender (better deliverability than `onboarding@resend.dev`); Turnstile `600010` can still block visitors on hardened/degraded browsers (WebGL/privacy) — drop Turnstile for honeypot-only if it matters. Remaining landing nice-to-haves (deferred): testimonials, per-page OG images, blog/changelog, status page link.
...
- **System Health:** backend `./mvnw test` **1208/1208** (r417: +9 print-agent pairing tests); frontend `build` clean, `lint` 0 errors / 16 pre-existing warnings, `test:run` **118/118** (r412 verified — `backend-types.ts` regen still pending, lands in T5); `landing` `pnpm build` clean (**26 pages**), ES/EN i18n parity 445/445.
...
- **Prod deploy:** tag `v*` on `main` → `backend-image.yml` builds the image → `./deploy/deploy.sh <tag>` from Cloud Shell (pure gcloud/IAP). Frontend auto-deploys from `main` via Cloudflare Pages. Backend currently on **`v0.2.2`** (deployed 2026-09-08, V1–V9 all applied).
...
- [ ] T2 agent — `StatusHub` observable, `dpapi/CredentialStore` (JNA Crypt32, machine scope) + JNA dep, `AgentPaths` (`%ProgramData%\EmberAgent`), `AgentConfig.resolve` (credential first, `agent.properties` fallback). Closes F-24
- [ ] T3 agent — `PairingClient.redeem`, `WindowsPrinterEnumerator` (`Get-Printer`, inkjet guess), `DiscoveredPrintersClient`, report on connect/refetch
- [ ] T4 agent — Swing `AgentDashboard` + `AgentTrayIcon` (clone `HubTrayIcon`) + `PairDialog`; extract `AgentRunner` from `Main`; `--headless` / `--tray` flags; `StatusHub` fed by `PrintJobHandler`
- [ ] T5 frontend — `CreateAgentModal` shows pairing code (key folded), `PrintingSettings` "Nuevo código" + Emparejado/Sin emparejar badge + `.exe` download link, `AddPrinterModal` `windowsQueueName` → `<select>` from `discoveredPrinters` + auto-`DRIVER` for inkjet, i18n ES/EN
- [ ] T6 installer — `printing-agent/build-installer.ps1` + `installer/EmberAgent.iss` + `jlink-modules.txt` + icon (clone `ember-hub/`), `{commonstartup}` shortcut, `%ProgramData%\EmberAgent\`, CI `build-print-agent` job (fat jar + `jpackage` app-image)
- [ ] T7 manual verification — `printing-agent/VERIFY.md` checklist on a clean Windows PC without Java + report
...
- [ ] **Security debt:** F-15 (Hub activation returns `adminPasswordHash` — redesign contract + migration path for installed Hubs); F-24 (print-agent key plaintext on disk — **fixed by EMB-PRINT-AGENT T2**, DPAPI); F-10/E-23 (PIN-login enumeration oracle — accepted, needs a product decision); F-22 (secret in git history — accepted, rotate if revisited).
