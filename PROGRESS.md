...
- **Last Completed Task:** report 549 — **EMPTY-STATES-FIRST-RUN** — new shared `EmptyState` component wired into 6 confirmed blank-screen gaps (`admin/inventory` + its 3 subtabs, `kitchen/orders`, `waiter/Tables`' main grid) that a brand-new tenant hits on first login. `admin/staff`/`admin/analytics`/`admin/cashRegister` already had their own empty-state handling, left untouched. `pnpm run test:run` **213/213** (61 files, +16 new), `build`/`lint` clean. Commit/push pending (user-gated, next tool call).
- **Prev (compressed, r548, same session):** **ADMIN-PASSWORD-RESET (F-25)** — operator-assisted reset for a locked-out restaurant ADMIN (Console "Resetear contraseña" → temp password + forced change-on-next-login modal); no mail infra exists in the backend, so self-service email reset was evaluated and deferred. `./mvnw test` 1540/1540. Two ideas from the same workflow review were discussed and explicitly **dropped**: unifying the Hub with `printing-agent` via a floatingNav panel (real friction turned out low — printing-agent is already a one-time install + auto-start tray process, not worth the engineering cost right now) and reopening a closed cash shift with a reason field (would reopen an already-committed financial record; a preview-before-commit redesign was proposed instead if ever revisited, see report's conversation — not built).
- **Prev (compressed, r540-547, 2026-09-23 push/tag/deploy round):** WAITER-ADD-ITEM review passes 1-4 (r544-547). Pushed `main` to `origin`. Discovered `v0.3.0` was tagged but never rolled out via `deploy.sh`; bumped `backend/pom.xml` → tagged/pushed **`v0.3.1`**. **Pending (user-gated, Cloud Shell):** `./deploy/deploy.sh 0.3.1` to actually roll out to the VM; `EmberHubSetup-0.3.0.exe` built locally, not published.
- **Current Active Task:** none in code. Prod-infra hardening (rollback rehearsal + restore test, see memory `ember-prod-infra-hardening`) and `deploy.sh 0.3.1` remain the open user-directed threads — next natural step per the pilot-readiness discussion this session.
...
- **Prod deploy:** tag `v*` on `main` → `backend-image.yml` builds `ember-backend:<tag-without-v>` → `./deploy/deploy.sh <tag-without-v>` from Cloud Shell (pure gcloud/IAP; the `v` is stripped — `deploy.sh 0.3.1`, not `v0.3.1`; no `gcloud` available on the local dev machine, this step is Cloud-Shell-only). Frontend + landing auto-deploy from `main` via Cloudflare. **`v0.3.1` tagged + pushed 2026-09-23, image build triggered — `deploy.sh 0.3.1` not yet run, prod is still on whatever was last actually rolled out (last confirmed rollout was v0.2.5, 2026-09-15; v0.3.0 was tagged but rollout status unconfirmed).** V15 (restaurant deployment mode) + V16/V17 (token revocation) will run automatically once deployed; prod Flyway is not baselined (see below) — never pre-run migrations by hand.
