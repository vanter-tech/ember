...
- **Current Active Task:** landing contact form bring-up. PR #101 merged (`81ebbad7`) — r402 Ember Local page + r403 cookie banner + r404 `/info/seguridad` + r405–407 contact form/Worker. Follow-up **r408 on branch `fix/landing-contact-env`** (PR pending). After merge+deploy: owner sets the 2 Secrets on the `ember` Worker, then test `/contacto` in a browser where Turnstile issues a token. Open: Turnstile `600010` on the owner's machine.
