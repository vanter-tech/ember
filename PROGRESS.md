...
- **Last Completed Task:** report 509 — **HUB-BACKUP-RESTORE Task 1** (models, `HubVersion`, `BackupConfigStore`, `BackupArchive`; backend **1327/1327**). Before it, report 508 — **HOTFIX 0.2.6.1 prep**: `pom.xml` -> `0.2.6.1`, `py-6` on the 3 join cards (buttons were flush to the card edge; `Card` primitive has no vertical padding). Bundles r506+r507. Backend **1314/1314**, frontend **164/164**. **Pending (user-gated):** test `EmberHubSetup-0.2.6.1.exe` → PR/merge → tag `v0.2.6.1` + `deploy.sh` → `publish-installer.sh hub 0.2.6.1`.
...
- **System Health:** backend `./mvnw test` **1327/1327**, frontend `pnpm run test:run` **164/164**, both builds clean, `lint` 0 errors (16 pre-existing warnings). Baseline before LIVE-BUG-BATCH was backend **1300/1300**/frontend **142/142** (r487/r483); CASH-SHIFT-DENOMINATION-COUNT separately reached backend **1312/1312**/frontend **156/156** (r494) before merging with LIVE-BUG-BATCH's own additions.
