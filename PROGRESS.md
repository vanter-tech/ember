...
- **Last Completed Task:** report 402 — **Landing: dedicated Ember Local page**. New `/info/local` (+ `/en/info/local`) in `InfoLayout`: *cómo funciona* (one PC as server, browser terminals on the LAN, `.exe` installer, tray icon), *funciona sin internet* (offline daily ops + opportunistic cloud sync + grace period, no numbers), *tus datos en tu servidor*, *actualizaciones y soporte*, and a concrete **Requisitos** table (Win 10/11 64-bit, dedicated server PC, local Wi-Fi, browser terminals, ESC/POS printer, internet optional). Added to the `/info` sidebar + a card on `/info`. Home band + `/funcionalidades` callout CTAs repointed from `/planes#local` → `/info/local`; `local.cta` reworded. `/funcionalidades` callout now spells out the 2 cloud-vs-Local diffs. `LocalPlan.astro` gained a secondary link → `/info/local` (price framing unchanged). `i18n/ui.ts` +45 keys/locale, ES/EN parity 406/406. `landing` `pnpm build` clean (**22 pages**). Branch `feat/landing-ember-local`.
- **Prev:** report 401 — **Landing: add Ember Local (on-premise)**. `EmberLocal.astro` (home band after `<Compare/>`) + `LocalPlan.astro` (`/planes` `#local`) + `/funcionalidades` callout; `local.*` i18n (×36) ES/EN. Positions the Hub as "Ember Local": staff-operated, no diner phones, runs on the restaurant LAN. Billing = annual **or** semi-annual license + quoted install; CTA → `/contacto` (no public price). Branch `feat/landing-ember-local`.
- **Prev:** report 400 — **Hub v2 prep: residual hardening** (3 items from the 399 audit): (1) `GlobalExceptionHandler` maps `OptimisticLockingFailureException` → 409 `code:"CONCURRENT_MODIFICATION"`; (2) **`isHubBuild` is now a function** (`VITE_HUB_BUILD` canonical, `BASE_URL` fallback); ~10 call sites; `frontend/.env.hub` + `build:hub --mode hub`; CI job `build-hub`; (3) `V9` → `ADD COLUMN IF NOT EXISTS`. **Not done:** real prod Flyway baseline (ops task). Branch `spec/hub-waiter-seats`.
- **Prev:** report 399 — **post-audit hardening on `spec/hub-waiter-seats`** (5 fixes): `@Transactional` on seat endpoints; `App.hubBuild.test.tsx` de-flaked; `addSeat` 409s at capacity; cruft removed; **public guest code-join** `/join` (`JoinByCode`).
- **Current Active Task:** none. **`feat/landing-ember-local` = PR #100 (open).** Reports 401 + 402 both on that branch. Next: merge #100; then **Hub v2** (Tauri shell — prep in 400; needs its own spec/plan) or **F-21** hardcoded creds.
...
- **System Health:** backend `./mvnw test` **1196/1196** (+1 optimistic-lock 409); frontend `build` **and `build:hub`** + `lint` clean (16 pre-existing warnings), `test:run` **118/118**; `landing` `pnpm build` clean (**22 pages**), ES/EN i18n parity 406/406.
