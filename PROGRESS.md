...
- **Last Completed Task:** report 540 — **PRICING-TABLE-HONESTY** (branch `fix/pricing-table-honesty` off `main`, not pushed) — audited every `/planes` comparison-table row and plan-card bullet against the one real source of truth (`PlanGateService`'s `requirePlanAtLeast`/`requireTableCapacity` call sites). Only 6 real gates exist site-wide: `tables`, `periodfilters`, `cashclose`, `roles` (KITCHEN/ACCOUNTANT only), `branding`, `export`. Removed 3 fully-fabricated rows/bullets with zero code backing (multi-waiter concurrency limit, multiple rooms/areas, multi-branch), fixed 5 rows that were marked as paid-tier-exclusive but are actually unrestricted for every tier (floor management, bill split, printing, "advanced" analytics — `getProducts`/`getTables` have no gate at all), relabeled the roles row (only blocks Kitchen/Accountant, not Waiter/Admin). Pro's card went from 4 bullets to 3 honest ones — export + unlimited tables is genuinely all that's Pro-exclusive today. `landing/src/lib/plans.ts` + `i18n/ui.ts` (ES+EN); `pnpm run build` clean, 28 pages. **Pending (user-gated):** PR/merge. Also this session: defined Ember Hub's pricing anchor ($350/yr, $210/semester list; 30% founder discount, first 12 months only, never published) — see memory `ember-hub-pricing`; paused a prod-infra hardening brainstorm before writing its spec — see memory `ember-prod-infra-hardening`.
...
- **Current Active Task:** none in code. This report is on its own branch, pending PR/merge. The F-14/F-15/F-17/F-21/F-10/RESTAURANT-DEPLOYMENT-MODE batch (r534-539) is already on `origin/main` (pushed 2026-09-22, bypassed 3 required status checks — **confirm CI still goes green on it**). Prod-infra hardening (see memory) and the pilot's Hub pricing conversation are both open threads outside the code, not yet executed. HUB-BACKUP-RESTORE and LIVE-BUG-BATCH's hotfix gates remain pending as above — one task per context, `/clear` between.
