...
- **System Health:** backend `./mvnw test` **1199/1199** (+3 in r410); frontend `build` clean, `lint` 0 errors / 16 pre-existing warnings, `test:run` **118/118**; `landing` `pnpm build` clean (**26 pages**), ES/EN i18n parity 445/445.
...
- [x] **Frontend UX batch (r409)** — `/admin` lands on analytics not inventory; section tours are request-only (no auto-start); arqueo difference no longer flashes on shift close (`CloseShiftDialog` unmounted with `cashShiftCurrent`); "Registrar movimiento" button + `MovementDialog` removed from waiter caja.
