...
- **Last Completed Task:** report 403 — **Landing: remove the cookie consent banner**. Deleted `CookieBanner.tsx` (only mounted on the home page) + its `index.astro` import/mount + `cookie.*` i18n keys (es/en). Plausible is cookieless and PROD-only, so the "Aceptar"-only banner gated nothing. Privacy §4 unchanged. `pnpm build` clean, ES/EN parity 403/403. Branch `feat/landing-ember-local`.
- **Prev:** report 402 — **Landing: dedicated Ember Local page** `/info/local` (+ `/en/…`) in `InfoLayout`: cómo funciona / offline + cloud sync / datos en tu servidor / updates+soporte / **Requisitos** table. Added to `/info` sidebar + card. Home band + `/funcionalidades` callout CTAs → `/info/local`; callout lists 2 cloud-vs-Local diffs; `LocalPlan` secondary link → `/info/local`. PR #101.
...
- **Current Active Task:** landing nice-to-have batch on `feat/landing-ember-local` (3 atomic commits): **[1 DONE report 403] remove cookie banner** · [2] `/info/seguridad` page · [3] real contact form on `/contacto` (Cloudflare Pages Function + Resend + Turnstile + `/gracias`). Then PR. PR #101 (Ember Local page) still open.
...
- **System Health:** backend `./mvnw test` **1196/1196** (+1 optimistic-lock 409); frontend `build` **and `build:hub`** + `lint` clean (16 pre-existing warnings), `test:run` **118/118**; `landing` `pnpm build` clean (**22 pages**), ES/EN i18n parity 403/403.
