...
- **Last Completed Task:** report 407 — **Landing: contact endpoint moved to a Worker**. `ember.vanter.net` is a **Cloudflare Worker + static assets** (`landing/wrangler.jsonc`, Workers Builds from GH `/landing`), NOT Pages — so `functions/api/contact.ts` (a Pages Functions convention, reports 405/406) would 404. Replaced with `landing/worker/index.ts` (`export default { fetch }`): `POST /api/contact` → same handler; else → `env.ASSETS.fetch`. `wrangler.jsonc` gains `main` + `assets.binding:"ASSETS"`; `functions/` deleted. `ContactForm.tsx` `SITE_KEY` now defaults to the real public Turnstile site key `0x4AAAAAAEsaAHQ6XDMni_IM` (env-overridable) so no build var needed. `wrangler deploy --dry-run` OK; `pnpm build` clean (26 pages). **Owner still sets 3 Worker runtime vars:** `TURNSTILE_SECRET_KEY` + `RESEND_API_KEY` (Secret) + `CONTACT_TO` (Text) — dashboard → `ember` → Settings → Variables and Secrets.
...
- **Current Active Task:** none. `feat/landing-ember-local` = **PR #101** (r402 Ember Local page + r403 cookie-banner removal + r404 `/info/seguridad` + r405/406/407 contact form → Worker). All pushed. Contact form is code-complete; **owner owes 3 Worker runtime vars** (`TURNSTILE_SECRET_KEY`, `RESEND_API_KEY`, `CONTACT_TO`) on the `ember` Worker before it delivers.
