...
- **Last Completed Task:** report 547 — **WAITER-ADD-ITEM-FOURTH-REVIEW-PASS**, plus the session's push/tag/deploy round (2026-09-23) — r546's `onInteractOutside` guard was incomplete (missed the panel's own close button, a focus-on-unmount path a target-containment check can't catch). Real fix: the cart panel is now a genuine DOM child of `DialogContent`, not a floating sibling — no guard needed, height matches the dialog exactly by construction, plus a slide-to-center open animation. **Live-verified in Chrome**: real waiter session, real table, confirmed via `getComputedStyle`/`getBoundingClientRect` that closing the panel no longer closes the modal and height matches pixel-for-pixel; cleanly torn down after. `pnpm run test:run` **192/193** (`AddItemModal.test.tsx` 16/16; 1 pre-existing unrelated `MenuJoin` flake). `build`/`lint` clean. Hub `.exe` rebuilt (r547's code).
- **Push/tag/deploy (2026-09-23):** pushed `main` to `origin` (r540-547, bypassed 3 required status checks). Discovered `v0.3.0` was already tagged against an *earlier* commit (right after RESTAURANT-DEPLOYMENT-MODE, before the F-10/14/15/17/21 security batch) — never actually rolled out via `deploy.sh`. Bumped `backend/pom.xml` 0.3.0→0.3.1 (direct commit, no report — pure release packaging), tagged and pushed **`v0.3.1`** to cover everything now on `main`. `backend-image.yml` CI build triggered. **Pending (user-gated, needs Cloud Shell — no local `gcloud`):** run `./deploy/deploy.sh 0.3.1` to actually roll it out to the VM; `EmberHubSetup-0.3.0.exe` (contains r547's fix) is local-only, not published to the public download bucket.
- **Prev (compressed, r544-546, merged to `main` locally 2026-09-22):** WAITER-ADD-ITEM-REVIEW-FEEDBACK (r544) — modal `6xl`, dish photos, dedicated "+" button. SECOND-REVIEW-PASS (r545) — bigger cards, `cursor-pointer`, bottom strip → floating panel (found buggy). THIRD-REVIEW-PASS (r546) — first (incomplete) attempt at the close-everything fix, destructive delete button, left-side qty badge; fully fixed in r547.
- **Prev (compressed, reports 534-542 — security-debt batch + RESTAURANT-DEPLOYMENT-MODE + 3 landing tasks):** F-21/F-14/F-15/F-17/F-10 security fixes + RESTAURANT-DEPLOYMENT-MODE `V15` (r534-539). PRICING-TABLE-HONESTY, DOWNLOADS-BUTTON-ALIGNMENT, HUB-PRICE-MODAL (r540-542). Also this session: defined Ember Hub's pricing + sales cadence — see memory `ember-hub-pricing`; paused a prod-infra hardening brainstorm — see memory `ember-prod-infra-hardening`. Full detail in each report + Task Queue Status below.
- **Current Active Task:** none in code. All of `main` (through r547 + the version bump) is on `origin/main`, tagged `v0.3.1`, CI image build triggered — **actual prod rollout (`deploy.sh 0.3.1`) still needs to run from Cloud Shell**. Prod-infra hardening remains the next user-directed thread — see memory `ember-prod-infra-hardening`. HUB-BACKUP-RESTORE and LIVE-BUG-BATCH's hotfix gates remain pending as above — one task per context, `/clear` between.
