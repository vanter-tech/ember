...
- **Last Completed Task:** task-4.4 (enforce `Restaurant.status` in `jwtAuthFilter`; ADMIN self-service `Restaurant.plan` change via `RestaurantAdminController`) — report 38
- **Current Active Task:** none — task queue exhausted, awaiting next backlog item
- **Predecessor Task:** task-4.3
- **System Health:** Frontend `pnpm run build` PASSING (0 TS errors); `pnpm run test:run` (Vitest) PASSING (1/1); `pnpm run lint` runs with 19 pre-existing errors/6 warnings (tracked in later tasks); Backend `./mvnw test` 423/423 passing (+12 for task-4.4: `RestaurantServiceTest`, `RestaurantAdminControllerTest`, `RestaurantStatusEnforcementTest`). Frontend not touched/rebuilt this task.
...
- task-3.6/3.7/3.8/4.1/4.2: `GET /kitchen/orders`/`GET /catalog/items` return `Page<T>` (frontend consumes it via a hand-written `Page<T>` type in `api.ts`, not regenerated); password complexity validation added; `SettingsPayload` extended with `paymentGateway`/`businessHours`/`billing.taxRules` (additive). Vitest lives inside `vite.config.ts`'s `test` block, no separate config file.
- Restaurant tenant-facing surfaces: task-4.3 added `GET /public/restaurants/{slug}/branding` (permitAll, `PublicRestaurantController`) which resolves slug→id via `RestaurantRepository` then binds/clears `TenantContextHolder` around `SettingService.getSettings` — the ONLY other place besides the JWT filters allowed to touch `TenantContextHolder`, don't add a second one; frontend `/t/:slug` → `TenantLanding.tsx` (path-based, not real subdomains). task-4.4 made `Restaurant.plan`/`status` (previously dead data) load-bearing: `SecurityConfig.jwtAuthFilter` now 403s every authenticated request when the resolved tenant's `status != ACTIVE`; `RestaurantAdminController` (`GET`/`PATCH /admin/restaurant/plan`, ADMIN-gated) lets a tenant self-serve plan changes. `RestaurantService.updateStatus` exists but is intentionally NOT wired to any controller — letting a tenant's own ADMIN flip their own `status` would let a suspended tenant unsuspend itself; it's the hook a future billing webhook calls. Don't add a self-service status endpoint without a non-tenant-controlled auth path.
...
- [x] **task-4.4:** Wire `Restaurant.plan`/`status` to a subscription-billing integration (billing the tenant, distinct from the existing diner-facing `billing` module).