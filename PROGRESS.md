...
- **System Health:** backend `./mvnw test` **1199/1199**; frontend `build` clean, `lint` 0 errors / 16 pre-existing warnings, `test:run` **118/118** (r412 verified); `landing` `pnpm build` clean (**26 pages**), ES/EN i18n parity 445/445.
...
- [ ] T1 backend — `V10` (`pairing_codes` + `discovered_printers` jsonb), `createPairingCode`/`redeemPairingCode`/`saveDiscoveredPrinters`, `POST /printing/admin/agents/{id}/pairing-code` + `/printing/agents/pair` (rate-limited) + `/printing/agents/me/discovered-printers`, `PrintAgentResponse` += `paired`/`discoveredPrinters`, `SecurityAuditTest`, regen types
