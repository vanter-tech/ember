...
- **Last Completed Task:** report 541 — **DOWNLOADS-BUTTON-ALIGNMENT** (branch `fix/downloads-button-alignment` off `main`, not pushed) — `/info/descarga`'s Hub/print-agent CTA buttons landed at different heights. Fixed with `mt-auto` on the button (same pattern `PlanCards.astro` already uses), relying on the grid's default `align-items: stretch` to keep both cards equal height. `landing/src/pages/info/descarga.astro` only — EN page is a re-export. `pnpm run build` clean. **Pending (user-gated):** PR/merge. Sibling `fix/pricing-table-honesty` (r540) also touches `landing/` — trivial merge expected.
