...
- **Last Completed Task:** report 546 — **WAITER-ADD-ITEM-THIRD-REVIEW-PASS** (on `main`, not yet pushed) — live testing of r545's cart panel found a real bug: its close button (and any click inside it) closed the *whole* modal, because the panel is a floating sibling outside Radix's Dialog content node, so Radix's outside-pointerdown dismiss logic treated it as clicking outside. Fixed with `onInteractOutside` on `DialogContent` guarded by a ref to the panel. Also: panel now anchors to the dialog's real measured `getBoundingClientRect()` (not an assumed `6xl` width) with `maxHeight` capped to the dialog's own height — fixes both "panel doesn't shrink to content" and "the pair isn't aligned" in one change; delete button now reuses `<Button variant="destructive" size="icon">` (same as `TableInformation.tsx`'s item-delete); quantity moved to a left-side padded circle badge (`{{qty}}x`) instead of inline text after the name. TDD: 1 new test reproducing the close-everything bug via `fireEvent.pointerDown` inside the panel, confirmed RED (whole DOM emptied) against r545's component, then GREEN. One real lint finding (`react-hooks/set-state-in-effect`) fixed along the way. `pnpm run test:run` **193/193**, `build`/`lint` clean. **Pending (user-gated):** commit + merge, rebuild `EmberHubSetup-0.3.0.exe` again.
- **Prev (compressed, r544-545, merged to `main` locally 2026-09-22):** WAITER-ADD-ITEM-REVIEW-FEEDBACK (r544) — modal `6xl`, more spacing, dish photos, dedicated "+" add button. WAITER-ADD-ITEM-SECOND-REVIEW-PASS (r545) — bigger cards, `cursor-pointer` everywhere, bottom cart strip replaced by a floating panel (later found buggy, fixed in r546). Hub `.exe` rebuilt after both rounds (each now superseded by r546's).
