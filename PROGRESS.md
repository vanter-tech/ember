...
- **Current Active Task:** **LIVE-BUG-BATCH** — 7 live user bug reports (2026-09-18), each its own report+commit on branch `fix/live-bug-batch-2026-09-18` off `origin/main`. 3/7 done (reports 488-490). Remaining 4: (4) Settings tab cards → grid layout matching `BrandingSettings`; (5) cash-shift closed-shift payments table should show table number not participant name; (6) ADMIN sees "no open shift" in `/waiter/tables` (r473→r487 regression, ADMIN never restored); (7) delete-restaurant button silently 500s — confirmed root cause: `restaurants_status_check` CHECK constraint never widened for `DELETED` (same class of bug as V11's `users_role_check`).
...
### LIVE-BUG-BATCH — 7 live user bug reports (2026-09-18) — branch `fix/live-bug-batch-2026-09-18` off `origin/main`
...
- [ ] 4 — Settings tab cards should use `BrandingSettings`'s grid layout (user-confirmed reference) instead of a narrow single column with wasted right-side space
- [ ] 5 — Admin Corte Z's closed-shift payments list shows the paying participant's name; should show the table (as a `<Table>`: Mesa/Monto/Método/Estado)
- [ ] 6 — ADMIN sees "no open shift" visiting `/waiter/tables` — `CashShiftController.current()` is `ACCOUNTANT,WAITER`-only, ADMIN was dropped in the r473 role migration and never restored
- [ ] 7 — Console "Eliminar restaurante" (slug typed, restaurant SUSPENDED) does nothing — confirmed live 500, root cause: `restaurants_status_check` never widened for `DELETED`; frontend also has no `onError` toast (silent failure either way)
