...
- **Current Active Task:** none in flight. **Next: HUB-BACKUP-RESTORE Task 2** (plan approved-pending; see Task Queue Status) - one task per context, `/clear` between. LIVE-BUG-BATCH is done (reports 495-504) on `fix/live-bug-batch-2026-09-18`, not yet PR'd/merged; hotfix 0.2.6.1 gates (r508) still pending.
...
- [ ] Task 2 - `PostgresTools` + `HubBackupService` backup/list/inspect/prune/scheduled-if-due (report 510)
- [ ] Task 3 - orchestrator `stopAndWait` + `restore` + real-Postgres round-trip test (report 511)
- [ ] Task 4 - `BackupScheduler` + 6 `HubControlServer` routes + `EmberApplication` wiring (report 512)
- [ ] Task 5 - Hub UI: types/api/`Modal`/`BackupCard` + tests (report 513)
- [ ] Task 6 - mount in `Dashboard`, `VERIFY.md` items 14-19, full verification (report 514); afterwards rebuild the Hub installer + run VERIFY 14-19
...
Seven independent bugs reported by the user in one message; investigated and root-caused up front, then fixed one per report/commit. Not a formal spec/plan doc (matches the lighter "Live QA findings Q1-Q6" precedent, reports 389-395). Report numbers renumbered from an original 488-497 to resolve a collision with CASH-SHIFT-DENOMINATION-COUNT (see below), which independently used the same range and merged to `main` first.
- [x] 1 — QR join (`MenuJoin.tsx`) forced a name-confirmation screen even for an authenticated real account; should only ask a guest — report 495
- [x] 2 — Waiter's per-table activity feed never shows "X left the table" (`SessionActivity.Type` has no `PARTICIPANT_LEFT`) — report 496
- [x] 3 — Table auto-closed (last participant leaves, nothing ordered) shows a misleading "Mesa pagada y cerrada" banner + leave confirmation should warn this will free the table — report 497
- [x] 4 — Settings tab cards should use `BrandingSettings`'s grid layout (user-confirmed reference) instead of a narrow single column with wasted right-side space — report 498
- [x] 5 — Admin Corte Z's closed-shift payments list shows the paying participant's name; should show the table (as a `<Table>`: Mesa/Monto/Método/Estado) — report 499
- [x] 6 — ADMIN sees "no open shift" visiting `/waiter/tables` — `CashShiftController.current()` is `ACCOUNTANT,WAITER`-only, ADMIN was dropped in the r473 role migration and never restored — report 500
- [x] 7 — Console "Eliminar restaurante" (slug typed, restaurant SUSPENDED) does nothing — confirmed live 500, root cause: `restaurants_status_check` never widened for `DELETED`; frontend also had no `onError` toast (silent failure either way) — report 501, **all 7 original bugs complete**. **Not yet live-verified** (needs real Postgres/deploy).
- [x] Follow-up 1 — Settings tabs' double-card frame (outer container + inner `Card` both drew a frame) + enable/disable toggle rows get a bordered box — report 502
- [x] Follow-up 2 — Settings tabs missing the header/content divider line (`BrandingSettings` reference) — report 503
...
- [ ] **HUB-BACKUP-RESTORE** - planned; tasks tracked in its own section above (spec + plan linked there). Restore = upload file from the dashboard, USB-recommended modal, settings covered by the Postgres dump.
