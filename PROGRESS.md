...
- **Last Completed Task:** report 538 — **F-17-TOKEN-REVOCATION** (branch `fix/jwt-token-revocation` off `main`, not pushed) — mitigates security finding **F-17** (JWT in `localStorage`, no server-side revocation): `users.token_version` (`V17`, idempotent) backs a new `ver` JWT claim; `SecurityConfig.jwtAuthFilter` rejects a token whose `ver` is behind the account's live version, next to the existing `isEnabled()` check (same DB round-trip, no session store added). `UserAdminService.setPin`/`clearPin`/deactivate bump it; new ADMIN endpoint `POST /admin/staff/{id}/revoke-sessions` for an explicit "sign out everywhere". TDD throughout. Backend **1519/1519** (0 failures this run, incl. the 2 usually-flaky port-59999 tests). Storage location (`localStorage`) and the 24h TTL are explicitly out of scope (bigger, cross-cutting change — STOMP handshake, CORS, CSRF). **Pending (user-gated):** PR/merge. F-10/E-23 (PIN-enumeration oracle) mitigation is a separate, not-yet-started task.
- **Prev (compressed, reports 534-537 — F-14/F-15/F-21 security fixes + RESTAURANT-DEPLOYMENT-MODE, all merged to `main` locally 2026-09-22, not yet pushed):** F-21 Hub Postgres/MinIO random credentials + real local auth (r536); F-14 `/platform/**` per-operator RBAC (r535); F-15 Hub generates its own admin password instead of receiving `adminPasswordHash` from the cloud (r537); RESTAURANT-DEPLOYMENT-MODE per-restaurant Web/Hub mode, `V15`, still awaits end-to-end manual acceptance + tag `v0.3.0` rollout (r534). Full detail in each report + Task Queue Status below.
...
- **Current Active Task:** none in code. F-17 (this report) and the F-14/F-15/F-21/RESTAURANT-DEPLOYMENT-MODE batch (r534-537) are all merged to `main` locally, pending PR/merge and push to `origin` (user-gated). F-10/E-23 mitigation not started. HUB-BACKUP-RESTORE and LIVE-BUG-BATCH's hotfix gates remain pending as above — one task per context, `/clear` between.
...
- **System Health:** backend `./mvnw test` **1519/1519** (this session, `fix/jwt-token-revocation` off `main`). Frontend/`ember-hub/ui` last verified in reports 537/538's predecessor batch: frontend `pnpm run build` clean, `ember-hub/ui` **34/34**.
...
- [ ] **Security debt:** ~~F-15 (Hub activation returns `adminPasswordHash`)~~ — **FIXED, report 537**; ~~F-21 (Hub Postgres/MinIO hardcoded credentials + local `trust` auth)~~ — **FIXED for new installs, report 536** (already-installed Hubs still need a dedicated migration, see its Task Queue entry); ~~F-14 (no RBAC on `/platform/**`)~~ — **FIXED, report 535**; ~~F-17 (JWT in `localStorage`, no revocation)~~ — **MITIGATED, report 538** (`tokenVersion` claim + revoke-sessions endpoint; storage location/TTL still open, see its Task Queue entry); ~~F-24 (print-agent key plaintext on disk)~~ — **FIXED, report 418** (DPAPI machine-scope credential store); F-10/E-23 (PIN-login enumeration oracle — accepted, needs a product decision, see plan in report 538's predecessor conversation); F-22 (secret in git history — accepted, rotate if revisited). F-15/F-21/F-14/F-17 all merged to `main` locally 2026-09-22, pending PR/merge.
