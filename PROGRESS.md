...
- **Current Active Task:** **LIVE-BUG-BATCH** — 7 live user bug reports (2026-09-18), each its own report+commit on branch `fix/live-bug-batch-2026-09-18` off `origin/main`. 2/7 done (reports 488-489). Remaining 5: (3) misleading "paid and closed" banner when a table is freed with no bill + smarter leave-confirmation copy; (4) Settings tab cards → grid layout matching `BrandingSettings`; (5) cash-shift closed-shift payments table should show table number not participant name; (6) ADMIN sees "no open shift" in `/waiter/tables` (r473→r487 regression, ADMIN never restored); (7) delete-restaurant button silently 500s — confirmed root cause: `restaurants_status_check` CHECK constraint never widened for `DELETED` (same class of bug as V11's `users_role_check`). **Note for bug 3:** `TableInformation.closedstate.test.tsx` already asserts the exact "Mesa pagada y cerrada..." banner text that bug 3 must change — revisit that file, not just the component.
...
- [ ] 3 — Table auto-closed (last participant leaves, nothing ordered) shows a misleading "Mesa pagada y cerrada" banner + leave confirmation should warn this will free the table
