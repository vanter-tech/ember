...
- **Last Completed Task:** report 414 — **Info card polish**: logos `h-12/14` → `h-20/24`; Vanter logo swapped `.webp` → `.svg` (owner's vector, old `.webp` deleted); Ember mark `ember_logo_info.svg` root `<g>` fill `#000000` → `#8c1717` (brand red). Branch `feat/settings-info-logos-tweak`.
