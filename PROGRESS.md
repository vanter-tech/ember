...
- **Last Completed Task:** report 543 — **WAITER-ADD-ITEM-TOUCH-SPLIT-VIEW** (branch `feat/waiter-add-item-touch-split-view` off `main`, not pushed) — redesigned `AddItemModal.tsx` (waiter's "add item" flow) from a plain list + `<select>` into a touch-friendly 2-column split view for an Ember Hub prospect on a touch-screen PC: clients as chips on the left (badge = draft-cart count), categorized products with tabs on the right (categories derived client-side from `MenuItemResponse.category`, already on every item — no new API call). Interaction model (user-decided): **draft cart per client, confirmed all at once** — tapping a product adds it to the active client's local cart (modifier groups still open the existing picker step first), footer strip shows removable +/− chips, one "Confirmar pedido" submits every client's every line as one `addWaiterItem` call per unit — same backend contract as before, **zero backend changes**. TDD: 12 new tests, RED confirmed against the old component first. Frontend `pnpm run test:run` **187/188** (1 pre-existing unrelated failure, confirmed via stash-and-compare against `main`), `build` clean, `lint` 0 errors. **Pending (user-gated):** PR/merge.
